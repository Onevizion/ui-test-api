...
class Grid2Js extends Js {
    ...
    LockType getGridCellLockTypeByRowIndexAndColIndex(Long gridIdx, int rowIndex, int columnIndex) {
    ...
    List<String> getGridCellAllChildsFontColor(Long gridIdx, int rowIndex, int columnIndex) {
    ...
    @SuppressWarnings("unchecked")
    List<String> getGridCellAllChildsBackgroundColor(Long gridIdx, int rowIndex, int columnIndex) {
        return (List<String>) execJs2(
                "var colors = [];" + 
                "var element = gridArr[" + gridIdx + "].grid.cellByIndex(" + rowIndex + ", " + columnIndex + ").cell;" + 
